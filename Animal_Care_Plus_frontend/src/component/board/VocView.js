...
function VocView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`/api/post/${vocId}`);
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await axios.get(`/api/comment/${vocId}`);
  ...
  const handleAddComment = async () => {
    ...
    try {
      const response = await request(`/api/comment/${vocId}`, 'POST', { contents: newComment }, userToken);
